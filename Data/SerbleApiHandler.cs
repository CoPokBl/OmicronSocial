...
public static class SerbleApiHandler {
    ...
    public static async Task<SerbleApiResponse<User>> GetUser(string token) {
        ...
        try {
            response = await client.GetAsync(GlobalConfig.Config["serble_api_url"] + "account");
        }
        catch (Exception e) {
    ...
    public static async Task<SerbleApiResponse<(string, string)>> GetRefreshToken(string oauthCode) {
        ...
        HttpResponseMessage response;
        try {
            response = await client.PostAsync(GlobalConfig.Config["serble_api_url"] + 
                                              $"oauth/token/refresh?" +
                                              $"code={oauthCode}" +
                                              $"&client_id={GlobalConfig.Config["serble_client_id"]}" +
                                              $"&client_secret={GlobalConfig.Config["serble_client_secret"]}" +
    ...
    public static async Task<SerbleApiResponse<User>> RegisterUser(string username, string password, string recaptchaToken) {
        ...
        HttpResponseMessage response;
        try {
            response = await client.PostAsync(GlobalConfig.Config["serble_api_url"] + "account", new StringContent(new {
                username,
                password
    ...
    public static async Task<SerbleApiResponse<User>> EditUser(string token, AccountEditRequest[] edits) {
        ...
        try {
            response = await client.PatchAsync(
                GlobalConfig.Config["serble_api_url"] + "account", 
    ...
    public static async Task<SerbleApiResponse<PublicOAuthApp>> GetPublicAppInfo(string appId) {
        ...
        HttpResponseMessage response;
        try {
            response = await client.GetAsync($"{GlobalConfig.Config["serble_api_url"]}app/{appId}/public");
        }
        catch (Exception e) {
    ...
    public static async Task<SerbleApiResponse<string>> AuthorizeApp(string token, string appId, string scopeString) {
        ...
        try {
            response = await client.PostAsync($"{GlobalConfig.Config["serble_api_url"]}account/authorizedApps", 
                new StringContent(jsonInp, Encoding.UTF8, 
                "application/json"));
        }
        catch (Exception e) {
    ...
    public static async Task<SerbleApiResponse<double>> CheckReCaptcha(string token) {
        ...
        HttpClient client = new();
        HttpResponseMessage response;
        string url = $"{GlobalConfig.Config["serble_api_url"]}recaptcha";
        url += "?token=" + token;
        try {
    ...
    public static async Task<SerbleApiResponse<OAuthApp[]>> GetUsersApps(string token) {
        ...
        client.DefaultRequestHeaders.Add("SerbleAuth", "User " + token);
        try {
            response = await client.GetAsync($"{GlobalConfig.Config["serble_api_url"]}app");
        }
        catch (Exception e) {
    ...
    public static async Task<SerbleApiResponse<bool>> CreateOAuthApp(string token, PublicOAuthApp app) {
        ...
        try {
            response = await client.PostAsync($"{GlobalConfig.Config["serble_api_url"]}app", new StringContent(jsonInp, Encoding.UTF8, "application/json"));
        }
        catch (Exception e) {
    ...
    public static async Task<SerbleApiResponse<string[]>> GetUsersOwnedProducts(string token) {
        ...
        client.DefaultRequestHeaders.Add("SerbleAuth", "App " + token);
        try {
            response = await client.GetAsync($"{GlobalConfig.Config["serble_api_url"]}products");
        }
        catch (Exception e) {
            return new SerbleApiResponse<string[]>(false, "Failed: " + e);
        }
